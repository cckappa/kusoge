...
func _enter() -> void:
	...
	header_bar.set_title_text(cargo.name)
	...
	for ability in cargo.abilities:
	...
	for _button in get_tree().get_nodes_in_group("selected_ability_buttons"):
